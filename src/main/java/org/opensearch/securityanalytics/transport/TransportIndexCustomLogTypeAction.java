...
public class TransportIndexCustomLogTypeAction
    ...
    @Inject
    public TransportIndexCustomLogTypeAction(
        ...
        super(
                IndexCustomLogTypeAction.NAME,
                transportService,
                actionFilters,
                IndexCustomLogTypeRequest::new);
        this.client = client;
    ...
    protected void doExecute(
            Task task,
            IndexCustomLogTypeRequest request,
            ActionListener<IndexCustomLogTypeResponse> listener) {
        ...
        AsyncIndexCustomLogTypeAction asyncAction =
                new AsyncIndexCustomLogTypeAction(task, request, listener);
        asyncAction.start();
    }
    ...
    public void onCreateMappingsResponse(CreateIndexResponse response) {
        if (response.isAcknowledged()) {
            log.info(
                    String.format(
                            Locale.getDefault(), "Created %s with mappings.", LogTypeService.LOG_TYPE_INDEX));
    ...
    public void onUpdateMappingsResponse(AcknowledgedResponse response) {
        if (response.isAcknowledged()) {
            log.info(
                    String.format(
                            Locale.getDefault(), "Updated  %s with mappings.", LogTypeService.LOG_TYPE_INDEX));
            IndexUtils.customLogTypeIndexUpdated();
        } else {
            log.error(
                    String.format(
    ...
    class AsyncIndexCustomLogTypeAction {
        ...
        void start() {
            ...
            try {
                if (!customLogTypeIndices.customLogTypeIndexExists()) {
                    customLogTypeIndices.initCustomLogTypeIndex(
                            new ActionListener<>() {
                                @Override
                ...
                if (space.equals("Sigma")) {
                    onFailures(
                            new OpenSearchStatusException(
                                    String.format(
                                            Locale.getDefault(),
                                            "Log Type with id %s cannot be updated because space is sigma",
                                            logTypeId),
                                ...
                                if (response.isTimedOut()) {
                                    onFailures(
                                            new OpenSearchStatusException(
                                                    String.format(
                                                            Locale.getDefault(),
                                                            "Search request timed out. Log Type with id %s cannot be updated",
                                                            logTypeId),
                                ...
                                if (response.getHits().getTotalHits().value() != 1) {
                                    onFailures(
                                            new OpenSearchStatusException(
                                                    String.format(
                                                            Locale.getDefault(),
                                                            "Log Type with id %s cannot be updated",
                                                            logTypeId),
                                                    RestStatus.INTERNAL_SERVER_ERROR));
                                    return;
                                }
                                                        ...
                                                        if (response.getHits().getTotalHits().value() > 0) {
                                                            onFailures(
                                                                    new OpenSearchStatusException(
                                                                            String.format(
                                                                                    Locale.getDefault(),
                                                                                    "Name of Log Type with id %s cannot be updated because active detectors exist",
                                                                                    logTypeId),
                                                                            RestStatus.BAD_REQUEST));
                                                            return;
                                                        }
                            ...
                            public void onResponse(Void unused) {
                                QueryBuilder queryBuilder =
                                        QueryBuilders.boolQuery()
                                                .must(
                                                        QueryBuilders.matchQuery("name", request.getCustomLogType().getName()))
                                                .must(
                                                        QueryBuilders.matchQuery(
                                                                "space", request.getCustomLogType().getSpace()));
                                                ...
                                                if (noOfHits > 0) {
                                                    onFailures(
                                                            new OpenSearchStatusException(
                                                                    String.format(
                                                                            Locale.ROOT,
                                                                            "Log Type with name %s already exists",
                                                                            request.getCustomLogType().getName()),
                                                                    RestStatus.INTERNAL_SERVER_ERROR));
                                                    return;
                                                }
                                                MaxAggregationBuilder queryBuilder =
                                                        AggregationBuilders.max("agg").field("tags.correlation_id");
                                                SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
                                                        ...
                                                        new ActionListener<>() {
                                                            ...
                                                            public void onResponse(SearchResponse response) {
                                                                ...
                                                                try {
                                                                    ...
                                                                    request
                                                                            .getCustomLogType()
                                                                            .setTags(Map.of("correlation_id", value + 1));
                                                                    ...
                                                                    IndexRequest indexRequest =
                                                                            new IndexRequest(LogTypeService.LOG_TYPE_INDEX)
                                                                                    .id(id)
        ...
        void checkRuleIndexAndUpdateLogType(CustomLogType existingLogType, String logTypeId) {
            if (ruleIndices.ruleIndexExists(false)) {
                                ...
                                if (response.getHits().getTotalHits().value() > 0) {
                                    onFailures(
                                            new OpenSearchStatusException(
                                                    String.format(
                                                            Locale.getDefault(),
                                                            "Name of Log Type with id %s cannot be updated because active rules exist",
                                                            logTypeId),
                                                    RestStatus.BAD_REQUEST));
                                    return;
                                }
                ...
                log.warn(
                        "Custom rule index missing, allowing update of custom log type {} to go through",
                        logTypeId);
                updateLogType(existingLogType, logTypeId);
            }
        }
        ...
        private void searchLogTypes(String logTypeId, ActionListener<SearchResponse> listener) {
            ...
            SearchRequest searchRequest =
                    new SearchRequest(LogTypeService.LOG_TYPE_INDEX)
                            .source(
                            ...
                            ActionRunnable.supply(
                                    ...
                                    () -> {
                                        if (t != null && t.length > 0) {
                                            ...
                                            return new IndexCustomLogTypeResponse(
                                                    logType.getId(), logType.getVersion(), RestStatus.CREATED, logType);
                                        }
                                    }));
        }
    }
