...
public class TransportIndexCorrelationRuleAction
    ...
    public TransportIndexCorrelationRuleAction(
        ...
        super(
                IndexCorrelationRuleAction.NAME,
                transportService,
                actionFilters,
                IndexCorrelationRuleRequest::new);
        this.client = client;
    ...
    protected void doExecute(
            ...
            ActionListener<IndexCorrelationRuleResponse> listener) {
        ...
        AsyncIndexCorrelationRuleAction asyncAction =
                new AsyncIndexCorrelationRuleAction(request, listener);
        asyncAction.start();
    }

    /** Parses the correlation rule from the raw body a REST-originated request carries. */
    private CorrelationRule parseCorrelationRule(IndexCorrelationRuleRequest request)
            throws IOException {
    ...
    class AsyncIndexCorrelationRuleAction {
        ...
        AsyncIndexCorrelationRuleAction(
                IndexCorrelationRuleRequest request,
                ActionListener<IndexCorrelationRuleResponse> listener) {
        ...
        void indexCorrelationRule() throws IOException {
            ...
            if (request.getMethod() == RestRequest.Method.POST) {
                indexRequest =
                        new IndexRequest(CorrelationRule.CORRELATION_RULE_INDEX)
                    ...
                    new ActionListener<>() {
                        ...
                        public void onResponse(IndexResponse response) {
                            if (response.status().equals(RestStatus.CREATED)
                                ...
                                onFailures(
                                        new OpenSearchStatusException(
                                                response.toString(), RestStatus.INTERNAL_SERVER_ERROR));
                            }
                        }
        ...
        private void onCreateMappingsResponse(CreateIndexResponse response) throws IOException {
            if (response.isAcknowledged()) {
                log.info(
                        String.format(
                                Locale.ROOT, "Created %s with mappings.", CorrelationRule.CORRELATION_RULE_INDEX));
