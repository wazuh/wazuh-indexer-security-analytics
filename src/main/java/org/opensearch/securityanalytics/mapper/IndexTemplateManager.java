...
public class IndexTemplateManager {
    ...
    public static String OPENSEARCH_SAP_COMPONENT_TEMPLATE_PREFIX =
            ".opensearch-sap-alias-mappings-component-";
    public static String OPENSEARCH_SAP_INDEX_TEMPLATE_PREFIX =
    ...
    public void upsertIndexTemplateWithAliasMappings(
        ...
        if (concreteIndexName == null) {
            String[] concreteIndices =
                    indexNameExpressionResolver.concreteIndexNames(
                            state, IndicesOptions.LENIENT_EXPAND_OPEN, indexName);
        ...
        Optional<CreateMappingResult> createMappingResult =
                createMappingResults.stream().filter(e -> e.getConcreteIndexName().equals(cin)).findFirst();
        ...
        upsertComponentTemplate(
                indexName, client, state, mappings, upsertComponentTemplateStepListener);

        upsertComponentTemplateStepListener.whenComplete(
                acknowledgedResponse -> {

                    // Find template which matches input index best. starts by directly matching with input
                    // index and
                    // if not found matches with current write index.
                    String templateName =
                            MetadataIndexTemplateService.findV2Template(
                                    state.metadata(), normalizeIndexName(indexName), false);
                    if (templateName == null) {
                        templateName =
                                MetadataIndexTemplateService.findV2Template(
                                        state.metadata(), normalizeIndexName(cin), false);
                    }

                    if (templateName == null) {
                        // If we find conflicting templates(regardless of priority) and that template was
                        // created by us,
                        // we will silently update index_pattern of that template.
                        // Otherwise, we will fail since we don't want to change index_pattern of user created
                        // index template
                        ...
                        // If there is 1 conflict which we own (SAP), we will update that template's
                        // index_pattern field
                        if (conflictingTemplates.size() == 1) {
                            ...
                            if (conflictingTemplateName.startsWith(OPENSEARCH_SAP_INDEX_TEMPLATE_PREFIX)) {
                                ...
                                String errorMessage =
                                        "Found conflicting template: [" + conflictingTemplateName + "]";
                                log.error(errorMessage);
                                actionListener.onFailure(
                                        SecurityAnalyticsException.wrap(new IllegalStateException(errorMessage)));
                            }
                        } else if (conflictingTemplates.size() > 1) {
                            String errorMessage =
                                    "Found conflicting templates: ["
                    ...
                    if (templateName == null) {
                        ...
                        upsertIndexTemplate(client, true, template, templateName, actionListener);
                    } else {
                        ...
                        if (template.composedOf().contains(componentName) == false) {
                            ...
                            try {
                                ...
                                upsertIndexTemplate(client, false, template, templateName, actionListener);
                            } catch (IOException e) {
        ...
        try {
            ...
            ComponentTemplate componentTemplate =
                    new ComponentTemplate(
                            new Template(null, new CompressedXContent(mappingsJson), null), 0L, null);
    ...
    public void deleteAllUnusedTemplates(ActionListener<Void> actionListener) {
        Map<String, ComposableIndexTemplate> allSapTemplates =
                IndexTemplateUtils.getAllSapComposableIndexTemplates(clusterService.state());
        ...
        getDetectorInputsListener.whenComplete(
                allInputIndices -> {
                    StepListener<Map<String, ComposableIndexTemplate>> doDeleteUnusedTemplatesListener =
                            new StepListener<>();
                    doDeleteUnusedTemplates(
                            allSapTemplates, allInputIndices, doDeleteUnusedTemplatesListener);

                    doDeleteUnusedTemplatesListener.whenComplete(
                            deletedTemplates -> {
    ...
    private void doDeleteUnusedComponentTemplates(ActionListener<Void> actionListener) {
        Set<String> componentTemplates =
                IndexTemplateUtils.getAllSapComponentTemplates(clusterService.state());
        // Start from set of ALL SAP Component Templates and remove each found in
        ...
        clusterService
                ...
                .metadata()
                .templatesV2()
                .forEach((name, template) -> template.composedOf().forEach(componentTemplates::remove));
        ...
        GroupedActionListener deleteMultipleComponentTemplatesListener =
                new GroupedActionListener(
                        new ActionListener<Collection<AcknowledgedResponse>>() {
                            @Override
    ...
    private void doDeleteUnusedTemplates(
        ...
        while (templateIterator.hasNext()) {
            ...
            for (String index : allDetectorInputIndices) {
                ...
                // If any of index patterns of template matches input index, we can finish here and move to
                // next template
                if (template.indexPatterns().stream()
                        .anyMatch((pattern) -> Regex.simpleMatch(pattern, normalizeIndexName(index)))) {
                    matched = true;
                    break;
                }
            }
        ...
        GroupedActionListener deleteMultipleTemplatesListener =
                new GroupedActionListener(
                        new ActionListener<Collection<AcknowledgedResponse>>() {
                            @Override
                            public void onResponse(Collection<AcknowledgedResponse> responses) {
