...
public class AlertsService {
                                ...
                                monitorId -> {
                                    if (detector
                                        ...
                                        if (detector
                                                        ...
                                                        .equals(monitorId)
                                                || (detector.getRuleIdMonitorIdMap().containsKey("-1")
                                                        && detector.getRuleIdMonitorIdMap().get("-1").equals(monitorId))) {
                        ...
                        AlertsService.this.getAlertsByMonitorIds(
                                ...
                                new ActionListener<>() {
                                    ...
                                    public void onFailure(Exception e) {
                                        log.error(
                                                "Failed to fetch alerts for detectorId: " + detectorId, e.getMessage());
                    ...
                    public void onResponse(
                        ...
                        listener.onResponse(
                                new GetAlertsResponse(
                                        getAlertsResponse.getAlerts().stream()
                                                .map(
                                                        e ->
                                                                mapAlertToAlertDto(
                                                                        e, monitorToDetectorMapping.get(e.getMonitorId())))
    ...
    public void getAlerts(
        ...
        detectors.forEach(
                detector -> {
                    ...
                    detector
                            .getMonitorIds()
                            .forEach(monitorId -> monitorToDetectorMapping.put(monitorId, detector.getId()));
                    // all monitorIds
    ...
    public void getAlerts(
        ...
        GetAlertsRequest request =
                new GetAlertsRequest(
                        ...
                        alertIds,
                        boolQueryBuilder);
        AlertingPluginInterface.INSTANCE.getAlerts((NodeClient) client, request, actionListener);
    }
     ...
     */
    public void ackknowledgeAlerts(
            org.opensearch.commons.alerting.action.GetAlertsResponse getAlertsResponse,
        ...
        GroupedActionListener<AcknowledgeAlertResponse> listener =
                new GroupedActionListener<>(
                        new ActionListener<Collection<AcknowledgeAlertResponse>>() {
                            ...
                            public void onResponse(Collection<AcknowledgeAlertResponse> responses) {
                                ...
                                for (AcknowledgeAlertResponse acknowledgeAlertResponse : responses) {
                                    acks.addAll(
                                            acknowledgeAlertResponse.getAcknowledged().stream()
                                                    .map(a -> mapAlertToAlertDto(a, getDetectorResponse.getId()))
                                                    .collect(Collectors.toList()));
                                    fails.addAll(
                                            acknowledgeAlertResponse.getFailed().stream()
                                                    .map(a -> mapAlertToAlertDto(a, getDetectorResponse.getId()))
                                                    .collect(Collectors.toList()));
        ...
        for (Map.Entry<String, List<String>> entry : alertsByMonitor.entrySet()) {
            AlertingPluginInterface.INSTANCE.acknowledgeAlerts(
                    (NodeClient) client,
                    new AcknowledgeAlertRequest(
                            entry.getKey(), entry.getValue(), WriteRequest.RefreshPolicy.IMMEDIATE),
                    listener);
        }
    }
