...
public class AlertsIT extends SecurityAnalyticsRestTestCase {
    ...
    public void testGetAlerts_success() throws IOException {
        ...
        String alertId =
                (String)
                        ((ArrayList<HashMap<String, Object>>) getAlertsBody.get("alerts")).get(0).get("id");
        String detectorId =
                (String)
                        ((ArrayList<HashMap<String, Object>>) getAlertsBody.get("alerts"))
                                .get(0)
                                .get("detector_id");
        params = new HashMap<>();
        ...
        OpenSearchRestTestCase.waitUntil(
                () -> {
                    try {
                        ...
                        Response currGetAlertsResponse =
                                makeRequest(
                                        client(), "GET", SecurityAnalyticsPlugin.ALERTS_BASE_URI, alertParams, null);
    ...
    public void testAckAlertsWithInvalidDetector() throws IOException {
        ...
        try {
            ...
            Assert.assertEquals(
                    HttpStatus.SC_NOT_FOUND, ex.getResponse().getStatusLine().getStatusCode());
        }
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testGetAlerts_byDetectorType_multipleDetectors_success()
        ...
        String monitorId1 =
                ((List<String>)
                        ...
                        .get(0);
        // Detector 2 - NETWORK
        DetectorInput inputNetflow =
                new DetectorInput(
                        "windows detector for security analytics",
                        List.of("netflow_test"),
     ...
     * 2, 3; maxRule - 4, 5, 6, 7; minRule - 7) 2. Verifies monitor execution 3. Verifies alerts
     *
     * @throws IOException
     */
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testMultipleAggregationAndDocRules_alertSuccess()
            throws IOException, InterruptedException {
        String index = createTestIndex(randomIndex(), windowsIndexMapping());
        ...
        assertEquals(
                ...
                response
                        .getHits()
                        .getTotalHits()
                        .value()); // 5 for rules, 1 for match_all query in chained findings monitor
        ...
        Response getAlertsResponse =
                makeRequest(client(), "GET", SecurityAnalyticsPlugin.ALERTS_BASE_URI, params1, null);
        Map<String, Object> getAlertsBody = asMap(getAlertsResponse);
        // TODO enable asserts here when able
        Assert.assertEquals(
                1,
                getAlertsBody.get("total_alerts")); // 2 doc level alerts for each doc, 1 bucket level alert

        input =
                new DetectorInput("updated", List.of("windows"), detectorRules, Collections.emptyList());
                ...
                () -> {
                    ...
                    try {
                        Response alertsResponse =
                                makeRequest(
                                        client(), "GET", SecurityAnalyticsPlugin.ALERTS_BASE_URI, queryParams, null);
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void test_detectorWith1AggRuleAndTriggeronRule_updateWithSecondAggRule()
            throws IOException, InterruptedException {
        String index = createTestIndex(randomIndex(), windowsIndexMapping());
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testAlertHistoryRollover_maxAge_low_retention()
            throws IOException, InterruptedException {
        updateClusterSetting(ALERT_HISTORY_ROLLOVER_PERIOD.getKey(), "1s");
        updateClusterSetting(ALERT_HISTORY_MAX_DOCS.getKey(), "1000");
        updateClusterSetting(ALERT_HISTORY_INDEX_MAX_AGE.getKey(), "1s");
    ...
    public void testGetAlertsFromAllIndices() throws IOException, InterruptedException {
        ...
        client()
                .performRequest(
                        new Request(
                                "POST", DetectorMonitorConfig.getAlertsIndex(randomDetectorType()) + "/_refresh"));
