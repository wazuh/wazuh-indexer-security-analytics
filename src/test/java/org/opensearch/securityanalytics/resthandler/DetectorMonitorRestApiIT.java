...
public class DetectorMonitorRestApiIT extends SecurityAnalyticsRestTestCase {
    /**
     * 1. Creates detector with 5 doc prepackaged level rules and one doc level monitor based on the
     * given rules 2. Creates two aggregation rules and assigns to a detector, while removing 5
     * prepackaged rules 3. Verifies that two bucket level monitor exists 4. Verifies the findings
     *
     * @throws IOException
     */
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testRemoveDocLevelRuleAddAggregationRules_verifyFindings_success()
            throws IOException {
        ...
        SearchResponse response =
                executeSearchAndGetResponse(
                        DetectorMonitorConfig.getRuleIndex(randomDetectorType()), request, true);
        ...
        String monitorType =
                ((Map<String, String>)
                                entityAsMap(
                                                client()
                                                        .performRequest(
                                                                new Request("GET", "/_plugins/_alerting/monitors/" + monitorId)))
                                        .get("monitor"))
                        .get("monitor_type");
        ...
        DetectorInput input =
                new DetectorInput(
                        ...
                        List.of("windows"),
                        List.of(new DetectorRule(sumRuleId), new DetectorRule(avgTermRuleId)),
                        emptyList());
            ...
            monitorType =
                    ((Map<String, String>)
                                    entityAsMap(
                                                    client()
                                                            .performRequest(
                                                                    new Request("GET", "/_plugins/_alerting/monitors/" + id)))
                                            .get("monitor"))
                            .get("monitor_type");
        ...
        for (Map<String, Object> finding : findings) {
            Set<String> aggRulesFinding =
                    ((List<Map<String, Object>>) finding.get("queries"))
                            .stream().map(it -> it.get("id").toString()).collect(Collectors.toSet());
        ...
        String findingDetectorId =
                ((Map<String, Object>) ((List) getFindingsBody.get("findings")).get(0))
                        .get("detectorId")
                        .toString();
    ...
    /**
     * 1. Creates detector with 1 aggregation rule and one bucket level monitor based on the
     * aggregation rule 2. Creates 5 prepackaged doc level rules and one custom doc level rule and
     * removes the aggregation rule 3. Verifies that one doc level monitor exists 4. Verifies the
     * findings
     *
     * @throws IOException
     */
    ...
    public void testReplaceAggregationRuleWithDocRule_verifyFindings_success() throws IOException {
        ...
        input =
                new DetectorInput(
                        "windows detector for security analytics",
                        List.of("windows"),
                        List.of(new DetectorRule(randomDocRuleId)),
    ...
    /**
     * 1. Creates detector with prepackaged doc rules 2. Verifies that detector with doc level monitor
     * is created 3. Removes all rules and updates detector 4. Verifies that detector doesn't have
     * monitors attached
     *
     * @throws IOException
     */
    ...
    /**
     * 1. Creates detector with aggregation rule 2. Adds new aggregation rule 3. Updates a detector 4.
     * Verifies that detector has 2 custom rules attached 5. Execute monitors and verifies findings
     *
     * @throws IOException
     */
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    ...
    public void testAddNewAggregationRule_verifyFindings_success() throws IOException {
        ...
        assertEquals(
                1,
                ((Map<String, Map<String, List>>) inputArr.get(0))
                        .get("detector_input")
                        .get("custom_rules")
                        .size());
        ...
        DetectorInput newInput =
                new DetectorInput(
                        "windows detector for security analytics",
                        List.of("windows"),
                        List.of(new DetectorRule(maxRuleId), new DetectorRule(sumRuleId)),
                        emptyList());
        ...
        hits = executeSearch(Detector.DETECTORS_INDEX, request);
        hit = hits.get(0);
        Map<String, List> updatedDetectorMap =
                (HashMap<String, List>) (hit.getSourceAsMap().get("detector"));
        ...
        assertNotNull(getFindingsBody);
        // Two bucket monitors are executed and only one finding is generated since maxRule is not
        // fulfilling the trigger condition
    ...
    /**
     * 1. Creates detector with 2 aggregation rule assigned 2. Verifies that 2 custom rules exists 3.
     * Removes one rule and updates a detector 4. Verifies that detector has only one custom rule and
     * one bucket level monitor
     *
     * @throws IOException
     */
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    ...
    public void testDeleteAggregationRule_verifyFindings_success() throws IOException {
        ...
        List<DetectorRule> detectorRules =
                aggRuleIds.stream().map(DetectorRule::new).collect(Collectors.toList());
        DetectorInput input =
                new DetectorInput(
                        "windows detector for security analytics",
                        List.of("windows"),
        ...
        DetectorInput newInput =
                new DetectorInput(
                        "windows detector for security analytics",
                        List.of("windows"),
                        List.of(new DetectorRule(avgRuleId)),
                        emptyList());
                ...
                (Map<String, String>)
                        (entityAsMap(
                                        client()
                                                .performRequest(
                                                        new Request(
                                                                "GET", "/_plugins/_alerting/monitors/" + monitorIds.get(0)))))
                                .get("monitor");
    ...
    /**
     * 1. Creates detector with 2 aggregation and prepackaged doc level rules 2. Replaces one
     * aggregation rule with a new one 3. Verifies that number of rules is unchanged 4. Verifies
     * monitor types 5. Verifies findings
     *
     * @throws IOException
     */
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    ...
    public void testReplaceAggregationRule_verifyFindings_success() throws IOException {
        ...
        assertEquals(
                2, numberOfMonitorTypes.get(MonitorType.BUCKET_LEVEL_MONITOR.getValue()).intValue());
        ...
        for (Map<String, Object> finding : findings) {
            ...
            Set<String> findingRules =
                    queries.stream().map(it -> it.get("id").toString()).collect(Collectors.toSet());
    ...
    public void testMinAggregationRule_findingSuccess() throws IOException {
        ...
        List<DetectorRule> detectorRules =
                aggRuleIds.stream().map(id -> new DetectorRule(id)).collect(Collectors.toList());
        DetectorInput input =
                new DetectorInput(
                        "windows detector for security analytics",
                        List.of("windows"),
    ...
    /**
     * 1. Creates detector with aggregation and prepackaged rules (sum rule - should match docIds: 1,
     * 2, 3; maxRule - 4, 5, 6, 7; minRule - 7) 2. Verifies monitor execution 3. Verifies findings
     *
     * @throws IOException
     */
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    ...
    public void testMultipleAggregationAndDocRules_findingSuccess() throws IOException {
        ...
        List<DetectorRule> detectorRules =
                List.of(
                        new DetectorRule(sumRuleId),
                        new DetectorRule(maxRuleId),
                        new DetectorRule(minRuleId),
                        new DetectorRule(avgRuleId),
                        new DetectorRule(cntRuleId),
                        new DetectorRule(randomDocRuleId));
        ...
        assertEquals(
                ...
                response
                        .getHits()
                        .getTotalHits()
                        .value()); // 6 for rules, 1 for match_all query in chained findings monitor
        ...
        for (String monitorId : monitorIds) {
            ...
            if (MonitorType.DOC_LEVEL_MONITOR.getValue().equals(monitor.get("monitor_type"))
                    && false == monitor.get("name").equals(detector.getName() + "_chained_findings")) {
                int noOfSigmaRuleMatches =
                        ((List<Map<String, Object>>)
                                        ((Map<String, Object>) executeResults.get("input_results")).get("results"))
                                .get(0)
                ...
                for (String ruleId : aggRuleIds) {
                    Object rule =
                            (((Map<String, Object>)
                                            ((Map<String, Object>)
                                                            ((List<Object>)
                                                                            ((Map<String, Object>) executeResults.get("input_results"))
                                                                                    .get("results"))
                                                                    .get(0))
                                                    .get("aggregations"))
                                    .get(ruleId));
        ...
        assertEquals(
                5, numberOfMonitorTypes.get(MonitorType.BUCKET_LEVEL_MONITOR.getValue()).intValue());
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testCreateDetector_verifyWorkflowCreation_success_WithoutGroupByRulesInTrigger()
            throws IOException {
        ...
        List<SearchHit> hits = executeSearch(Detector.DETECTORS_INDEX, request);
        SearchHit hit = hits.get(0);
        Map<String, Object> detectorMap =
                (HashMap<String, Object>) (hit.getSourceAsMap().get("detector"));
        ...
        assertEquals(
                "Number of workflows not correct",
                1,
                ((List<String>) detectorMap.get("workflow_ids")).size());
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testCreateDetector_verifyWorkflowCreation_success_WithGroupByRulesInTrigger()
            throws IOException {
        updateClusterSetting(ENABLE_WORKFLOW_USAGE.getKey(), "true");
        String index = createTestIndex(randomIndex(), windowsIndexMapping());
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testUpdateDetector_disabledWorkflowUsage_verifyWorkflowNotCreated_success()
            throws IOException {
    ...
    public void testUpdateDetector_removeRule_verifyWorkflowUpdate_success() throws IOException {
        ...
        DetectorTrigger t1 =
                new DetectorTrigger(
                        ...
                        "1",
                        List.of(),
                        List.of(randomDocRuleId, maxRuleId),
                        List.of(),
                        List.of(),
                        List.of(),
                        List.of());
        ...
        DetectorInput newInput =
                new DetectorInput(
                        "windows detector for security analytics",
                        List.of("windows"),
                        Arrays.asList(new DetectorRule(randomDocRuleId)),
                        getRandomPrePackagedRules().stream()
                                .map(DetectorRule::new)
                                .collect(Collectors.toList()));
        detector = randomDetectorWithInputs(List.of(newInput));
        ...
        List<Map<String, Object>> monitorRunResults =
                (List<Map<String, Object>>) entityAsMap(executeResponse).get("monitor_run_results");
        assertEquals(1, monitorRunResults.size());

        int noOfSigmaRuleMatches =
                ((List<Map<String, Object>>)
                                ((Map<String, Object>) monitorRunResults.get(0).get("input_results"))
                                        .get("results"))
                        .get(0)
                        .size();
        assertEquals(6, noOfSigmaRuleMatches);
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testCreateDetector_verifyWorkflowExecutionBucketLevelDocLevelMonitors_success()
            throws IOException {
        updateClusterSetting(ENABLE_WORKFLOW_USAGE.getKey(), "true");
        String index = createTestIndex(randomIndex(), windowsIndexMapping());
        ...
        List<Map<String, Object>> monitorRunResults =
                (List<Map<String, Object>>) executeWorkflowResponseMap.get("monitor_run_results");
        ...
        for (Map<String, Object> runResult : monitorRunResults) {
            if (((Map<String, Object>) runResult.get("trigger_results")).get(maxRuleId) != null) {
                ...
                int noOfSigmaRuleMatches =
                        ((List<Map<String, Object>>)
                                        ((Map<String, Object>) runResult.get("input_results")).get("results"))
                                .get(0)
                                .size();
                // 5 prepackaged and 1 custom doc level rule
    ...
    @AwaitsFix(bugUrl = "")
    public void
            testCreateDetector_verifyWorkflowExecutionMultipleBucketLevelDocLevelMonitors_success_WithBucketLevelTriggersOnRuleIds()
                    throws IOException {
        String index = createTestIndex(randomIndex(), windowsIndexMapping());
        ...
        t2 =
                new DetectorTrigger(
                        ...
                        "1",
                        List.of(),
                        List.of(minRuleId, avgRuleId, cntRuleId),
                        List.of(),
                        List.of(),
                        List.of(),
                        List.of());
        ...
        List<String> monitorIds = ((List<String>) (detectorMap).get("monitor_id"));
        assertTrue(
                "Expected monitorIds size to be either 6 or 7",
                monitorIds.size() == 6 || monitorIds.size() == 7);
        ...
        HashMap<String, String> bucketMonitorsToRuleMap =
                (HashMap<String, String>) detectorMap.get("bucket_monitor_id_rule_id");
        ...
        for (Map<String, Object> runResult : monitorRunResults) {
            ...
            if (monitorId.equals(docMonitorId)) {
                ...
                Map<String, Object> trigger_results =
                        (Map<String, Object>) runResult.get("trigger_results");
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testCreateDetectorWithKeywordsRule_ensureNoFindingsWithoutTextMapping_success()
            throws IOException {
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testCreateDetectorWithKeywordsRule_ensureNoFindingsWithoutDateMapping_success()
            throws IOException {
    ...
    public void testCreateDetectorWithCloudtrailAggrRule() throws IOException {
        ...
        Assert.assertEquals(
                ...
                String.format(
                        Locale.getDefault(), "%s/%s", SecurityAnalyticsPlugin.DETECTOR_BASE_URI, createdId),
                createResponse.getHeader("Location"));
        Assert.assertFalse(
                ((Map<String, Object>) responseBody.get("detector")).containsKey("rule_topic_index"));
        Assert.assertFalse(
                ((Map<String, Object>) responseBody.get("detector")).containsKey("findings_index"));
        Assert.assertFalse(
                ((Map<String, Object>) responseBody.get("detector")).containsKey("alert_index"));

        String detectorTypeInResponse =
                (String) ((Map<String, Object>) responseBody.get("detector")).get("detector_type");
        Assert.assertEquals(
                "Detector type incorrect",
                randomDetectorType().toLowerCase(Locale.ROOT),
                detectorTypeInResponse);
        ...
        String workflowId =
                ((List<String>)
                                ((Map<String, Object>) hit.getSourceAsMap().get("detector")).get("workflow_ids"))
                        .get(0);
    ...
    public void testCreateDetectorWithCloudtrailAggrRuleWithEcsFields() throws IOException {
        ...
        createIndex(
                index,
                Settings.EMPTY,
                cloudtrailOcsfMappings(),
                "\"" + indexAlias + "\":{\"is_write_index\": true}");
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testCreateDetectorWithCloudtrailAggrRuleWithRolloverIndexAliases()
    ...
    private static void assertRuleMonitorFinding(
            Map<String, Object> executeResults,
            String ruleId,
            int expectedDocCount,
            List<String> expectedTriggerResult) {
                        ...
                        (((Map<String, Object>)
                                        ((Map<String, Object>)
                                                        ((Map<String, Object>)
                                                                        ((List<Object>)
                                                                                        ((Map<String, Object>)
                                                                                                        executeResults.get("input_results"))
                                                                                                .get("results"))
                                                                                .get(0))
                                                                .get("aggregations"))
                                                .get("result_agg"))
        ...
        List<String> triggerResultBucketKeys =
                ((Map<String, Object>)
                                ((Map<String, Object>)
                                                ((Map<String, Object>) executeResults.get("trigger_results")).get(ruleId))
                                        .get("agg_result_buckets"))
