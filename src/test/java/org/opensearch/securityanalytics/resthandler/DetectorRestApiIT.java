...
public class DetectorRestApiIT extends SecurityAnalyticsRestTestCase {
    ...
    private String setupDetector() throws IOException {
        ...
        // Execute CreateMappingsAction to add alias mapping for index
        final Request createMappingRequest =
                new Request("POST", SecurityAnalyticsPlugin.MAPPER_BASE_URI);
        // both req params and req body are supported
        createMappingRequest.setJsonEntity(
                "{ \"index_name\":\""
                        + index
        ...
        final DetectorTrigger detectorTrigger =
                new DetectorTrigger(
                        null,
                        "test-trigger",
                        "1",
        ...
        final Detector detector =
                randomDetectorWithTriggers(getRandomPrePackagedRules(), List.of(detectorTrigger));
    ...
    public void testCreatingADetectorScheduledJobFinding() throws IOException, InterruptedException {
        ...
        Detector detector =
                randomDetectorWithTriggersAndScheduleAndEnabled(
                        getRandomPrePackagedRules(),
                        List.of(
                                new DetectorTrigger(
                                        null,
    ...
    public void test_searchDetectors_detectorsIndexNotExists() throws IOException {
        try {
            makeRequest(
                    client(),
                    "DELETE",
                    SecurityAnalyticsPlugin.DETECTOR_BASE_URI + "/" + "d1",
                    Collections.emptyMap(),
                    null);
    ...
    public void testCreatingADetectorWithNonExistingCustomRule() throws IOException {
        ...
        DetectorInput input =
                new DetectorInput(
                        ...
                        List.of("windows"),
                        List.of(new DetectorRule(java.util.UUID.randomUUID().toString())),
    ...
    /**
     * 1. Creates detector with no rules 2. Detector without rules and monitors created successfully
     *
     * @throws IOException
     */
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    ...
    public void testGettingADetector() throws IOException {
        ...
        Response getResponse =
                makeRequest(
                        client(),
                        "GET",
                        SecurityAnalyticsPlugin.DETECTOR_BASE_URI + "/" + createdId,
                        Collections.emptyMap(),
                        null);
    ...
    public void testSearchingDetectors() throws IOException {
        ...
        List<Map<String, Object>> hits =
                ((List<Map<String, Object>>)
                        ((Map<String, Object>) searchResponseBody.get("hits")).get("hits"));
        Map<String, Object> hit = hits.get(0);
        String detectorTypeInResponse =
                (String) ((Map<String, Object>) hit.get("_source")).get("detector_type");
        Assert.assertEquals(
                "Detector type incorrect",
                detectorTypeInResponse.toLowerCase(Locale.ROOT),
                randomDetectorType().toLowerCase(Locale.ROOT));
    }
    ...
    public void testCreatingADetectorWithCustomRules() throws IOException {
        ...
        String detectorType =
                (String) ((Map<String, Object>) hit.getSourceAsMap().get("detector")).get("detector_type");
        Assert.assertEquals(
                "Detector type incorrect",
                detectorType.toLowerCase(Locale.ROOT),
                randomDetectorType().toLowerCase(Locale.ROOT));
    ...
    public void testCreatingADetectorWithAggregationRules() throws IOException {
        ...
        DetectorInput input =
                new DetectorInput(
                        "windows detector for security analytics",
                        List.of("windows"),
                        List.of(new DetectorRule(customAvgRuleId)),
                        getRandomPrePackagedRules().stream()
                                .map(DetectorRule::new)
                                .collect(Collectors.toList()));
        Detector detector = randomDetectorWithInputs(List.of(input));
        ...
        Assert.assertEquals(
                "Incorrect Location header",
                String.format(
                        Locale.getDefault(), "%s/%s", SecurityAnalyticsPlugin.DETECTOR_BASE_URI, detectorId),
                createResponse.getHeader("Location"));
        Assert.assertFalse(
                ((Map<String, Object>) responseBody.get("detector")).containsKey("rule_topic_index"));
        Assert.assertFalse(
        ...
        String firstMonitorType =
                ((Map<String, String>)
                                entityAsMap(
                                                client()
                                                        .performRequest(
                                                                new Request(
                                                                        "GET", "/_plugins/_alerting/monitors/" + firstMonitorId)))
                                        .get("monitor"))
                        .get("monitor_type");
        ...
        String secondMonitorType =
                ((Map<String, String>)
                                entityAsMap(
                                                client()
                                                        .performRequest(
                                                                new Request(
                                                                        "GET", "/_plugins/_alerting/monitors/" + secondMonitorId)))
                                        .get("monitor"))
                        .get("monitor_type");
        ...
        String thirdMonitorType =
                ((Map<String, String>)
                                entityAsMap(
                                                client()
                                                        .performRequest(
                                                                new Request(
                                                                        "GET", "/_plugins/_alerting/monitors/" + thirdMonitorId)))
                                        .get("monitor"))
                        .get("monitor_type");
        ...
        HashMap<String, Object> docLevelQuery =
                (HashMap<String, Object>) ((List<?>) finding.get("queries")).get(0);
        String ruleId = docLevelQuery.get("id").toString();
        // Verify if the rule id in bucket level finding is the same as rule used for bucket monitor
        // creation
        ...
        Response getResponse =
                makeRequest(
                        ...
                        Collections.emptyMap(),
                        null);
        String getDetectorResponseString =
                new String(getResponse.getEntity().getContent().readAllBytes());
    ...
    public void testAggRuleCount() throws IOException {
        ...
        DetectorInput input =
                new DetectorInput(
                        "windows detector for security analytics",
                        List.of("windows"),
                        List.of(new DetectorRule(customAggRule)),
                        getRandomPrePackagedRules().stream()
                                .map(DetectorRule::new)
                                .collect(Collectors.toList()));
        Detector detector = randomDetectorWithInputs(List.of(input));
        ...
        String bucketLevelMonitorId = "";
        Map<String, Object> monitorOpts =
                ((Map<String, Object>) (detectorAsMap).get("bucket_monitor_id_rule_id"));
        for (Map.Entry<String, Object> monitorOpt : monitorOpts.entrySet()) {
            if (!(monitorOpt.getKey().equals("-1")
                    || monitorOpt.getKey().equals("chained_findings_monitor"))) {
        ...
        Map<String, Object> executeResults =
                entityAsMap(executeAlertingMonitor(bucketLevelMonitorId, Collections.emptyMap()));
        Map<String, Object> trigger =
                (Map<String, Object>)
                        ((Map<String, Object>) executeResults.get("trigger_results"))
                                .entrySet()
                                .iterator()
                                .next()
                                .getValue();
    ...
    public void testUpdateADetector() throws IOException {
        ...
        String detectorTypeInResponse =
                (String)
                        ((Map<String, Object>) (asMap(updateResponse).get("detector"))).get("detector_type");
        Assert.assertEquals(
                "Detector type incorrect",
                randomDetectorType().toLowerCase(Locale.ROOT),
                detectorTypeInResponse);
    ...
    public void testDeletingADetector_single_ruleTopicIndex() throws IOException {
        ...
        Response deleteResponse =
                makeRequest(
                        client(),
                        "DELETE",
                        SecurityAnalyticsPlugin.DETECTOR_BASE_URI + "/" + detectorId1,
                        Collections.emptyMap(),
                        null);
        Assert.assertEquals("Delete detector failed", RestStatus.OK, restStatus(deleteResponse));
        // We deleted 1 detector, but 1 detector with same type exists, so we expect queryIndex to be
        // present
        Assert.assertTrue(
                doesIndexExist(
                        String.format(
                                Locale.ROOT, ".opensearch-sap-%s-detectors-queries-000001", "test_windows")));

        deleteResponse =
                makeRequest(
                        client(),
                        "DELETE",
                        SecurityAnalyticsPlugin.DETECTOR_BASE_URI + "/" + detectorId2,
                        Collections.emptyMap(),
                        null);
        Assert.assertEquals("Delete detector failed", RestStatus.OK, restStatus(deleteResponse));
    ...
    public void testDeletingADetector_oneDetectorType_multiple_ruleTopicIndex() throws IOException {
        ...
        Response response =
                makeRequest(
                        ...
                        Map.of(),
                        new StringEntity("{\"query\": {\"match\": {\"_id\": \"" + detectorId1 + "\"}}}"),
                        new BasicHeader("Content-Type", "application/json"));
        String ruleTopicIndex1 =
                ((Map<String, Object>)
                                        ((Map<String, Object>)
                                                        ((List<Map<String, Object>>)
                                                                        ((Map<String, Object>) responseAsMap(response).get("hits"))
                                                                                .get("hits"))
                                                                .get(0)
                                                                .get("_source"))
                                                .get("detector"))
                                .get("rule_topic_index")
                                .toString()
                        + "-000001";
        ...
        response =
                makeRequest(
                        ...
                        ".opensearch-sap-detectors-config/_search",
                        Map.of(),
                        new StringEntity("{\"query\": {\"match\": {\"_id\": \"" + detectorId2 + "\"}}}"),
                        new BasicHeader("Content-Type", "application/json"));
        String ruleTopicIndex2 =
                ((Map<String, Object>)
                                        ((Map<String, Object>)
                                                        ((List<Map<String, Object>>)
                                                                        ((Map<String, Object>) responseAsMap(response).get("hits"))
    ...
    public void testCreatingADetectorWithTimestampFieldAliasMapping() throws IOException {
        ...
        for (int idx = 0; idx < monitorIds.size(); ++idx) {
            ...
            String monitorTypeOpt =
                    ((Map<String, String>)
                                    entityAsMap(
                                                    client()
                                                            .performRequest(
                                                                    new Request(
                                                                            "GET", "/_plugins/_alerting/monitors/" + monitorIdOpt)))
                                            .get("monitor"))
                            .get("monitor_type");
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testCreatingADetectorWithTimestampFieldAliasMapping_verifyTimeRangeInBucketMonitor()
            throws IOException {
        String index = createTestIndex(randomIndex(), productIndexMapping());
        ...
        Assert.assertEquals(
                findings.size(),
                0); // there should be no findings as doc is not in time range of current run
    }
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testCreatingDetectorWithDynamicQueryIndexDisabledAndThenEnabledToUpdate()
            throws IOException {
        ...
        Assert.assertTrue(
                doesIndexExist(DetectorMonitorConfig.getRuleIndex(randomDetectorType()) + "-000001"));
    ...
    public void testDeletingDetectorEditedToHaveZeroRules() throws IOException {
        ...
        // Update the detector to have 0 rules. This should update the detector's 'monitor_id' attribute
        // to be an empty array.
        DetectorInput input =
                new DetectorInput("detector with no rules", List.of(index), List.of(), List.of());
        Detector updatedDetector = randomDetectorWithInputs(List.of(input));
