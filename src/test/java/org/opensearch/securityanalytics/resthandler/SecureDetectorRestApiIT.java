...
public class SecureDetectorRestApiIT extends SecurityAnalyticsRestTestCase {
    ...
    static Map<String, String> roleToPermissionsMap =
            Map.ofEntries(
                    Map.entry(
                            SECURITY_ANALYTICS_FULL_ACCESS_ROLE,
                            "cluster:admin/opendistro/securityanalytics/detector/*"),
                    Map.entry(
                            SECURITY_ANALYTICS_READ_ACCESS_ROLE,
                            "cluster:admin/opendistro/securityanalytics/detector/read"));
    ...
    public void testCreateDetectorWithFullAccess() throws IOException {
        try {
            ...
            // Enable backend filtering and try to read detector as a user with no backend roles matching
    ...
    public void testCreateDetectorWithNoBackendRoles() throws IOException {
        ...
        RestClient userFullClient =
                new SecureRestClientBuilder(
                                getClusterHosts().toArray(new HttpHost[] {}), isHttps(), userFull, password)
                        .setSocketTimeout(60000)
                        .build();
    ...
    public void testCreateDetector_userHasIndexAccess_success() throws IOException {
        ...
        try {
            clientWithAccess =
                    new SecureRestClientBuilder(
                                    getClusterHosts().toArray(new HttpHost[] {}), isHttps(), userWithAccess, password)
                            .setSocketTimeout(60000)
                            .build();
            String index =
                    createTestIndex(client(), randomIndex(), windowsIndexMapping(), Settings.EMPTY);
    ...
    public void testCreateDetector_userDoesntHaveIndexAccess_failure() throws IOException {
        ...
        try {
            clientWithoutAccess =
                    new SecureRestClientBuilder(
                                    getClusterHosts().toArray(new HttpHost[] {}),
                                    isHttps(),
                                    userWithoutAccess,
                                    password)
                            .setSocketTimeout(60000)
            ...
            try {
                ...
                assertEquals(
                        "Create detector error status", RestStatus.FORBIDDEN, restStatus(e.getResponse()));
            }
        } finally {
    ...
    public void testUpdateDetector_userDoesntHaveIndexAccess_failure() throws IOException {
        ...
        try {
            ...
            try {
                ...
                assertEquals(
                        "Update detector error status", RestStatus.FORBIDDEN, restStatus(e.getResponse()));
            }
        } finally {
            if (clientWithoutAccess != null) clientWithoutAccess.close();
            deleteUser(userWithoutAccess);
