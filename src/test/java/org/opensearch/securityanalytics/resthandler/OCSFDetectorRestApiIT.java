...
public class OCSFDetectorRestApiIT extends SecurityAnalyticsRestTestCase {
    ...
    public void testCloudTrailAPIActivityOCSFDetector() throws IOException {
        ...
        Detector detector =
                randomDetectorWithInputs(
                        List.of(
                                new DetectorInput(
                                        "ocsf logs based cloudtrail detector for security analytics",
                                        List.of(index),
                                        List.of(),
    ...
    public void testCloudTrailAPIActivityRawDetector() throws IOException {
        ...
        Detector detector =
                randomDetectorWithInputs(
                        List.of(
                                new DetectorInput(
                                        "raw logs based cloudtrail detector for security analytics",
                                        List.of(index),
                                        List.of(),
                                        getPrePackagedRules("cloudtrail").stream()
                                                .map(DetectorRule::new)
    ...
    public void testRoute53OCSFDetector() throws IOException {
        ...
        Response createResponse =
                makeRequest(
                        ...
                        Collections.singletonMap("category", "dns"),
        ...
        Detector detector =
                randomDetectorWithInputs(
                        List.of(
                                new DetectorInput(
                                        "raw logs based route53 detector for security analytics",
                                        List.of(index),
                                        List.of(new DetectorRule(createdId)),
                                        getPrePackagedRules("dns").stream()
                                                .map(DetectorRule::new)
                                                .collect(Collectors.toList()))),
                        "dns");
    ...
    public void testVpcflowOcsfDetector() throws IOException {
        ...
        Response createResponse =
                makeRequest(
                        ...
                        "POST",
                        SecurityAnalyticsPlugin.RULE_BASE_URI,
                        Collections.singletonMap("category", "vpcflow"),
                        new StringEntity(rule),
                        new BasicHeader("Content-Type", "application/json"));
        Assert.assertEquals("Create rule failed", RestStatus.CREATED, restStatus(createResponse));
        ...
        Detector detector =
                randomDetectorWithInputs(
                        List.of(
                                new DetectorInput(
                                        "raw logs based vpcflow detector for security analytics",
                                        List.of(index),
                                        List.of(new DetectorRule(createdId)),
                                        getPrePackagedRules("vpcflow").stream()
                                                .map(DetectorRule::new)
                                                .collect(Collectors.toList()))),
                        "vpcflow");
    ...
    public void testCloudtrailPrincipalIdAndArnFieldsGenerateFinding() throws IOException {
        ...
        Assert.assertEquals(
                "Detector type incorrect", "cloudtrail", detectorType.toLowerCase(Locale.ROOT));
