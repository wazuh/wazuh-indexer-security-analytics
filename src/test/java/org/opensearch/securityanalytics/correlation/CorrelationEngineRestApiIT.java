...
public class CorrelationEngineRestApiIT extends SecurityAnalyticsRestTestCase {
    ...
    public void testBasicCorrelationEngineWorkflow() throws IOException, InterruptedException {
        ...
        String finding =
                ((List<Map<String, Object>>) getFindingsBody.get("findings")).get(0).get("id").toString();

        OpenSearchRestTestCase.waitUntil(
                () -> {
                    try {
                        List<Map<String, Object>> correlatedFindings =
                                searchCorrelatedFindings(finding, "test_windows", 300000L, 10);
                        if (correlatedFindings.size() == 2) {
                            ...
                            for (var correlatedFinding : correlatedFindings) {
                                if (correlatedFinding.get("detector_type").equals("network")) {
                                    ...
                                    Assert.assertTrue(
                                            ((List<String>) correlatedFinding.get("rules")).contains(ruleId));
                                }
                            }
                            return true;
                        }
                ...
                () -> {
                    try {
                        ...
                        Request request =
                                new Request(
                                        "GET",
                                        "/_plugins/_security_analytics/correlations?start_timestamp="
                                                + startTime
                                                + "&end_timestamp="
                                                + endTime);
    ...
    @AwaitsFix(bugUrl = "")
    @SuppressWarnings("unchecked")
    public void testBasicCorrelationEngineWorkflowWithoutRules()
    ...
    @AwaitsFix(bugUrl = "")
    @SuppressWarnings("unchecked")
    public void testBasicCorrelationEngineWorkflowWithRolloverByMaxAge()
            throws IOException, InterruptedException {
        updateClusterSetting(SecurityAnalyticsSettings.ENABLE_AUTO_CORRELATIONS.getKey(), "true");
        updateClusterSetting(
                SecurityAnalyticsSettings.CORRELATION_HISTORY_ROLLOVER_PERIOD.getKey(), "1s");
        updateClusterSetting(
                SecurityAnalyticsSettings.CORRELATION_HISTORY_INDEX_MAX_AGE.getKey(), "1s");
                    ...
                    public boolean getAsBoolean() {
                        try {
                            ...
                            if (correlatedFindings.size() == 2) {
                                ...
                                Assert.assertTrue(
                                        "Did not find more then 2 correlation indices", correlationIndices.size() >= 2);
                                return true;
                            }
                            return false;
                        } catch (Exception ex) {
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testBasicCorrelationEngineWorkflowWithRolloverByMaxDoc()
            throws IOException, InterruptedException {
        updateClusterSetting(SecurityAnalyticsSettings.ENABLE_AUTO_CORRELATIONS.getKey(), "true");
        updateClusterSetting(
                SecurityAnalyticsSettings.CORRELATION_HISTORY_ROLLOVER_PERIOD.getKey(), "1s");
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testBasicCorrelationEngineWorkflowWithRolloverByMaxDocAndShortRetention()
            throws IOException, InterruptedException {
        updateClusterSetting(SecurityAnalyticsSettings.ENABLE_AUTO_CORRELATIONS.getKey(), "true");
        updateClusterSetting(
                SecurityAnalyticsSettings.CORRELATION_HISTORY_ROLLOVER_PERIOD.getKey(), "1s");
                ...
                () -> {
                    try {
                        ...
                        if (correlatedFindings.size() == 2) {
                            ...
                            updateClusterSetting(
                                    SecurityAnalyticsSettings.CORRELATION_HISTORY_RETENTION_PERIOD.getKey(), "1s");
                            updateClusterSetting(
                                    SecurityAnalyticsSettings.CORRELATION_HISTORY_MAX_DOCS.getKey(), "1000");
                            ...
                            Assert.assertTrue(
                                    "Found more than 1 correlation indices", correlationIndices.size() == 1);
                            return true;
                        }
                        return false;
                    } catch (Exception ex) {
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testBasicCorrelationEngineWorkflowWithFieldBasedRulesOnMultipleLogTypes()
            throws IOException, InterruptedException {
    ...
    public void testBasicCorrelationEngineWorkflowWithIndexPatterns()
            throws IOException, InterruptedException {
        updateClusterSetting(SecurityAnalyticsSettings.ENABLE_AUTO_CORRELATIONS.getKey(), "false");
    ...
    // TODO: Disabled due to commented-out REST endpoints. Re-enable when endpoints are restored.
    @AwaitsFix(bugUrl = "")
    public void testBasicCorrelationEngineWorkflowWithCustomLogTypes()
            throws IOException, InterruptedException {
        LogIndices indices = new LogIndices();
        ...
        Response createResponse =
                makeRequest(
                        ...
                        Collections.emptyMap(),
                        toHttpEntity(customLogType));
        Assert.assertEquals(
                "Create custom log type failed", RestStatus.CREATED, restStatus(createResponse));
        ...
        createResponse =
                makeRequest(
                        ...
                        "POST",
                        SecurityAnalyticsPlugin.RULE_BASE_URI,
                        Collections.singletonMap("category", customLogType.getName()),
        ...
        DetectorInput input =
                new DetectorInput(
                        "custom log type detector for security analytics",
                        List.of(index),
        ...
        String monitorId =
                ((List<String>)
                        ...
                        .get(0);
        String ruleId =
                createNetworkToCustomLogTypeFieldBasedRule(indices, customLogType.getName(), index);
        ...
        OpenSearchRestTestCase.waitUntil(
                () -> {
                    try {
                        List<Map<String, Object>> correlatedFindings =
                                searchCorrelatedFindings(finding, customLogType.getName(), 300000L, 10);
                        if (correlatedFindings.size() == 1) {
                            Assert.assertTrue(true);
