    ...
    public void testUpdateCorrelationRule() throws IOException {
        ...
        response =
                makeRequest(
                        client(),
                        "PUT",
                        SecurityAnalyticsPlugin.CORRELATION_RULES_BASE_URI + "/" + id,
                        Collections.emptyMap(),
                        toHttpEntity(rule));
        Assert.assertEquals(200, response.getStatusLine().getStatusCode());
        responseMap = responseAsMap(response);
        Assert.assertEquals(
                "custom-updated-rule", ((Map<String, Object>) responseMap.get("rule")).get("name"));
    }
    ...
    public void testDeleteCorrelationRule() throws IOException {
        ...
        response =
                makeRequest(
                        client(),
                        "POST",
                        SecurityAnalyticsPlugin.CORRELATION_RULES_BASE_URI + "/_search",
                        Collections.emptyMap(),
        ...
        Assert.assertEquals(
                1,
                Integer.parseInt(
                        ((Map<String, Object>) ((Map<String, Object>) responseMap.get("hits")).get("total"))
                                .get("value")
                                .toString()));
